...
<div class="row">
  ...
  <div class="table-responsive">
                    ...
                    <div class="row align-items-end">
                        ...
                        </div><a class="button is-info" href="{{ route('fournisseur.create') }}">Créer un  fournisseur</a>
                        </div>
                    </div>
                    <div class="row"><div class="col-sm-12 col-md-6">
                        ...
                        <tbody>
                            @foreach($fournisseurs as $fournisseur)
                                <tr>
                                    <td>{{ $fournisseur->ID_fourn }}</td>
                                    <td>{{ $fournisseur->Nom_Fournisseur }}</td>
                                    <td>{{ $fournisseur->Adresse_Fournisseur }}</td>
                                    <td>{{ $fournisseur->Telephone_Fournisseur }}</td>
                                    <td>{{ $fournisseur->Localite_Fournisseur }}</td>
                                    <td>{{ $fournisseur->Email_Fournisseur }}</td>
                                    <td>
                                        <a href="{{ route('fournisseur.show', $fournisseur->ID_fourn) }}">
                                            <i class="icon feather icon-eye f-w-600 f-16 m-r-15 text-c-green"></i>
                                        </a>
                                        <a href="{{ route('fournisseur.edit',$fournisseur->ID_fourn) }}">
                                            <i class="feather icon-edit f-w-600 f-16 text-c-red"></i>
                                        </a>
                                        <form action="{{ route('fournisseur.destroy', $fournisseur->ID_fourn) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button  type="submit" class="btn  btn-primary">
                                                <i class="feather icon-trash-2 f-w-600 f-16 text-c-white"></i>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
