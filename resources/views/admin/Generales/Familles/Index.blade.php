...
<div class="row">
  ...
  <div class="table-responsive">
                    ...
                    <div class="row align-items-end">
                        ...
                        </div><a class="button is-info" href="{{ route('famille.create') }}">Créer une  famille</a>
                        </div>
                    </div>
                    <div class="row"><div class="col-sm-12 col-md-6">
                        ...
                        <tbody>
                            @foreach($familles as $famille)
                                <tr>
                                    <td>{{ $famille->ID_fam }}</td>
                                    <td>{{ $famille->Nom_famille }}</td>
                                    <td>
                                        <a href="{{ route('famille.show', $famille->ID_fam) }}">
                                            <i class="icon feather icon-eye f-w-600 f-16 m-r-15 text-c-green"></i>
                                        </a>
                                        <a href="{{ route('famille.edit',$famille->ID_fam) }}">
                                            <i class="feather icon-edit f-w-600 f-16 text-c-red"></i>
                                        </a>
                                        <form action="{{ route('famille.destroy', $famille->ID_fam) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button  type="submit">
