                    ...
                    <div class="row align-items-end">
                        ...
                        <div class="col">
                            <select class=" col-sm-12"  onchange="window.location.href = this.value">
                                    <option value="{{ route('direction.index') }}" @unless($Nom_Appartement) selected @endunless>Tous les Appartements</option>
                                    @foreach($appartements as $appartement)
                                    <option value="{{ route('direction.appartement', $appartement->Nom_appartement) }}" {{ $Nom_Appartement == $appartement->Nom_appartement ? 'selected' : '' }}>{{ $appartement->Nom_appartement }}</option>
                                    @endforeach
                            </select>
                        ...
                        </div><a class="button is-info" href="{{ route('direction.create') }}">Créer un une Direction</a>
                        </div>
                    </div>
                    <div class="row"><div class="col-sm-12 col-md-6">
                        ...
                        <tbody>
                            @foreach($directions as $direction)
                                <tr>
                                    <td>{{ $direction->ID_dir }}</td>
                                    <td>{{ $direction->Nom_de_la_direction }}</td>
                                    <td>{{ $direction->Code_direction }}</td>
                                    <td>{{ $direction->Contact }}</td>
                                    <td>{{ $direction->Nom_du_directeur }}</td>
                                    <td>{{ $direction->Description_activité }}</td>
                                    <td>{{ $direction->Commentaire }}</td>
                                    <td>
                                        <a href="{{ route('direction.show', $direction->ID_dir) }}">
                                            <i class="icon feather icon-eye f-w-600 f-16 m-r-15 text-c-green"></i>
                                        </a>
                                        <a href="{{ route('direction.edit',$direction->ID_dir) }}">
                                            <i class="feather icon-edit f-w-600 f-16 text-c-red"></i>
                                        </a>
                                        <form action="{{ route('direction.destroy', $direction->ID_dir) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button  class="btn btn-primary btn-sm btn-round" type="submit">
