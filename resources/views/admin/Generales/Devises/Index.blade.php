...
<div class="row">
  ...
  <div class="table-responsive">
                    ...
                    <div class="row align-items-end">
                        ...
                        </div><a class="button is-info" href="{{ route('devise.create') }}">Créer un  devise</a>
                        </div>
                    </div>
                        ...
                        <tbody>
                            @foreach($devises as $devise)
                                <tr>
                                    ...
                                    <td>{{ $devise->Nom_devise }}</td>
                                    <td>{{ $devise->Taux_conversion }}</td>
                                    <td>{{ $devise->Defaut }}</td>

                                    <td>
                                        <a href="{{ route('devise.show', $devise->ID_devise) }}">
                                            <i class="icon feather icon-eye f-w-600 f-16 m-r-15 text-c-green"></i>
                                        </a>
                                        <a href="{{ route('devise.edit',$devise->ID_devise) }}">
                                            <i class="feather icon-edit f-w-600 f-16 text-c-red"></i>
                                        </a>
                                        <form action="{{ route('devise.destroy', $devise->ID_devise) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button  type="submit">
